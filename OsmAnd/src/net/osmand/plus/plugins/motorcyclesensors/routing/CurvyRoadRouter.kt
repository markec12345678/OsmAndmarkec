...
class CurvyRoadRouter {
	...
	/**
	 * Build a RouteStatistics object representing the curviness breakdown of a route.
	 *
	 * This integrates with OsmAnd's existing route statistics infrastructure
	 * (RouteStatisticsHelper) so that curviness can be displayed alongside
	 * surface type, steepness, etc. in the route details UI.
	 *
	 * @param routeSegments The calculated route segments
	 * @return RouteStatistics object with curviness partition data
	 */
	fun buildCurvinessRouteStatistics(routeSegments: List<RouteSegmentResult>): RouteStatistics? {
		if (routeSegments.isEmpty()) {
			return null
		}

		val elements = mutableListOf<RouteSegmentAttribute>()
		var prevBearing: Float? = null
		var prevAttribute: RouteSegmentAttribute? = null
		var totalDistance = 0f
		...
		for (segment in routeSegments) {
			...
			if (prevBearing != null) {
				val twistiness = calculateTwistiness(prevBearing, currentBearing, segmentDistance)
				val classification = classifyRoadTwistiness(twistiness)
				val propertyName = classificationToPropertyName(classification)

				val currentAttribute = RouteSegmentAttribute(propertyName, classification.color, -1)
				currentAttribute.incrementDistanceBy(segmentDistance)

				// Merge with previous segment if same classification
				if (prevAttribute != null && prevAttribute.propertyName == currentAttribute.propertyName) {
					prevAttribute.incrementDistanceBy(segmentDistance)
				} else {
		...
		// Build partition map
		val partition = linkedMapOf<String, RouteSegmentAttribute>()
		for (attr in elements) {
			val existing = partition[attr.userPropertyName]
			if (existing != null) {
				existing.incrementDistanceBy(attr.distance)
			} else {
				partition[attr.userPropertyName] = RouteSegmentAttribute(attr)
			}
		}

		return RouteStatistics(ROUTE_INFO_CURVINESS, elements, partition, totalDistance)
	}
